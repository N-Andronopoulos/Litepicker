...
export class Litepicker extends Calendar {
  ...
  public show(el = null) {
    ...
    if (this.options.mobileFriendly && isMobile()) {
      ...
      const pickerBCR2 = this.picker.getBoundingClientRect();
      this.picker.style.top = `calc(50% - ${(pickerBCR2.height / 2)}px)`;
      this.picker.style.left = `calc(50% - ${(pickerBCR2.width / 2)}px)`;
      this.picker.style.right = null;
      this.picker.style.bottom = null;
      this.picker.style.zIndex = String(this.options.zIndex);
